...
public class SearchGUI {
	...
	public SearchGUI() throws TikaException, IOException, SAXException, CsvValidationException {
		initialize();
		...
		try {
			...
			if (startR != null && endR != null) {
				Rectangle2D expandedRect = new Rectangle2D.Double(startR.getX(), startR.getY(),
						endR.getX() - startR.getX() + endR.getWidth(), Math.max(startR.getHeight(), endR.getHeight()));
	...
	private void performSearch() {
		...
		try {
			...
			if (results.size() >= 2) {
				...
				String highlightText = shouldHighlightTopDoc ? "Best recommendation:\n" : "";
				String highlightLineJump = shouldHighlightTopDoc ? "\n" : "";
				String otherResultsHighlight = shouldHighlightTopDoc ? "Other results:\n" : "";

				addTextWithStyle(highlightText + topDoc.title() + " - Score: " + round(topDoc.score(), 3) + "\n"
				...
				for (int i = 1; i < results.size(); i++) {
					...
					addTextWithStyle(doc.title() + " - Score: " + round(doc.score(), 3) + "\n", false);
				}
			}
			else if (results.size() == 1) {
				DocumentRank topDoc = results.get(0);
				addTextWithStyle(topDoc.title() + " - Score: " + round(topDoc.score(), 3) + "\n", true);
			}
		}
		catch (Exception e) {
